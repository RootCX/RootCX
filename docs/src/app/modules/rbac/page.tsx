    ...
    <DocsLayout toc={toc}>
      <div className="flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-3">
          <h1 className="text-4xl font-bold tracking-tight">Roles & Permissions (RBAC)</h1>
          <p className="text-lg text-muted-foreground leading-7">
            Declarative, fine-grained access control with policy inheritance and row-level ownership filters, enforced securely at the Core level.
          </p>
        </div>
        ...
        <section className="flex flex-col gap-4" id="outcomes">
          ...
          <ul className="flex flex-col gap-2 text-muted-foreground text-sm leading-7">
            {[
              "Zero-middleware enforcement: Roles and policies are validated globally by Core, eliminating the risk of forgetting to secure an endpoint.",
              "Manifest-driven security: Define complex inheritance charts and permissions directly in your JSON manifest without writing a single line of security logic.",
              "Row-level ownership: Instantly scope read, update, and delete access for specific roles so users can only interact with their own data."
            ].map((item, i) => (
        ...
        <section className="flex flex-col gap-4" id="overview">
          ...
          <p className="text-muted-foreground leading-7">
            ...
            and Core enforces them automatically. There is no middleware to wire up and no guards to write — the
            enforcement layer lives inside Core itself.
          </p>
      ...
      },
      {
        ...
        <section className="flex flex-col gap-4" id="ownership">
          ...
          <ul className="flex flex-col gap-2 text-muted-foreground leading-7 list-none pl-0">
            ...
            <li className="flex items-start gap-2">
              <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-muted-foreground/50 shrink-0" />
              <span>On <strong className="text-foreground font-medium">create</strong>, Core automatically injects <code className="rounded bg-white/5 px-1.5 py-0.5 font-mono text-xs text-foreground">owner_id = current_user_id</code> into the INSERT statement. The client cannot override this value.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-muted-foreground/50 shrink-0" />
  ...
  "policies": [
    {
      ...
      "actions": ["create", "read", "update", "delete"]
        ...
        <section className="flex flex-col gap-4" id="role-inheritance">
          ...
          <p className="text-muted-foreground leading-7">
            Core resolves the full inheritance graph at policy evaluation time using a depth-first traversal
...
// users → read, update, delete`} />
          <p className="text-muted-foreground leading-7">
            When merging policies from inherited roles, Core takes the <strong className="text-foreground font-medium">union of all
  ...
  "permissions": {
    ...
    "roles": [...]
        ...
        <section className="flex flex-col gap-4" id="rbac-routes">
          ...
          <p className="text-muted-foreground leading-7">
            Core exposes a set of management endpoints for inspecting and mutating role assignments. All routes
          ...
          <h3 className="text-lg font-semibold text-foreground mt-2">List Roles</h3>
          <CodeBlock language="bash" code={`GET /api/v1/apps/{appId}/rbac/roles

curl http://localhost:9100/api/v1/apps/my-app/rbac/roles \\
...
[
  ...
  },
  {
    ...
    "inherits": []
          ...
          <h3 className="text-lg font-semibold text-foreground mt-2">List Assignments</h3>
          <CodeBlock language="bash" code={`GET /api/v1/apps/{appId}/rbac/assignments

curl http://localhost:9100/api/v1/apps/my-app/rbac/assignments \\
  -H "Authorization: Bearer <token>"

# Response (requires admin)
[
  { "userId": "user_abc123", "role": "admin",  "assignedAt": "2025-01-15T09:30:00Z" },
  { "userId": "user_def456", "role": "editor", "assignedAt": "2025-01-16T14:20:00Z" },
  { "userId": "user_ghi789", "role": "viewer", "assignedAt": "2025-01-17T11:00:00Z" }
]`} />

          <h3 className="text-lg font-semibold text-foreground mt-2">Assign Role</h3>
          <CodeBlock language="bash" code={`POST /api/v1/apps/{appId}/rbac/assign
...
curl -X POST http://localhost:9100/api/v1/apps/my-app/rbac/assign \\
  -H "Authorization: Bearer <token>" \\
...
{ "message": "role 'admin' assigned" }`} />
          ...
          <h3 className="text-lg font-semibold text-foreground mt-2">Revoke Role</h3>
          <CodeBlock language="bash" code={`POST /api/v1/apps/{appId}/rbac/revoke
Content-Type: application/json
...
curl -X POST http://localhost:9100/api/v1/apps/my-app/rbac/revoke \\
  -H "Authorization: Bearer <token>" \\
  -H "Content-Type: application/json" \\
  -d '{"userId": "user_abc123", "role": "admin"}'

# Response
{ "message": "role 'admin' revoked" }`} />

          <h3 className="text-lg font-semibold text-foreground mt-2">My Permissions</h3>
          <CodeBlock language="bash" code={`GET /api/v1/apps/{appId}/rbac/me

curl http://localhost:9100/api/v1/apps/my-app/rbac/me \\
  -H "Authorization: Bearer <token>"
...
# Response
{
  ...
  "permissions": {
    ...
    "users":    { "actions": ["read", "update", "delete"],           "ownership": false }
          ...
          <h3 className="text-lg font-semibold text-foreground mt-2">User Permissions</h3>
          <CodeBlock language="bash" code={`GET /api/v1/apps/{appId}/rbac/users/{userId}

curl http://localhost:9100/api/v1/apps/my-app/rbac/users/user_def456 \\
  -H "Authorization: Bearer <token>"
