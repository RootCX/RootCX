...
enum Cmd {
    ...
    #[command(override_help = COMPLETIONS_HELP)]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
Generate shell completion scripts for rootcx commands.

When installing rootcx through a package manager, it's possible that
...
Generate a `_rootcx` completion script and put it somewhere in your `$fpath`:

    rootcx completions zsh > /usr/local/share/zsh/site-functions/_rootcx
...
Generate a `rootcx.fish` completion script:
...
Add the line and save the file:
    ...
    Invoke-Expression -Command $(rootcx completions powershell | Out-String)
