...
export const setSecret = (key: string, value: string) =>
  send("/api/v1/platform/secrets", post("/api/v1/platform/secrets", { key, value }));

export const deleteSecret = (key: string) =>
  send(`/api/v1/platform/secrets/${encodeURIComponent(key)}`, { method: "DELETE" });

export const getAiConfig = () => json<AiConfig | null>("/api/v1/config/ai");

export const saveAiConfig = (config: AiConfig) =>
  send("/api/v1/config/ai", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(config) });

export const verifySchema = (manifest: unknown) =>
  json<SchemaVerification>("/api/v1/apps/schema/verify", post("", manifest));

export const syncManifest = (manifest: unknown) =>
  send("/api/v1/apps", post("", manifest));
