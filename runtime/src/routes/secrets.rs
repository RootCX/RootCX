...
pub async fn set_secret(_identity: Identity, State(rt): State<SharedRuntime>, Path(app_id): Path<String>, Json(body): Json<JsonValue>) -> Result<Json<JsonValue>, ApiError> {
...
pub async fn delete_secret(_identity: Identity, State(rt): State<SharedRuntime>, Path((app_id, key)): Path<(String, String)>) -> Result<Json<JsonValue>, ApiError> {
    let (pool, sm) = pool_and_secrets(&rt).await?;
...
pub async fn list_secrets(_identity: Identity, State(rt): State<SharedRuntime>, Path(app_id): Path<String>) -> Result<Json<Vec<String>>, ApiError> {
    let (pool, sm) = pool_and_secrets(&rt).await?;
