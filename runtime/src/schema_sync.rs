...
pub enum ColumnDiff {
    ...
    ReplaceCheckConstraint {
        column_name: String,
        old_constraint_names: Vec<String>,
        ...
        match db_map.get(field.name.as_str()) {
            ...
            Some(db_col) => {
                ...
                if want_not_null && !db_col.not_null {
                    changes.push(ColumnDiff::SetNotNull {
                        column_name: field.name.clone(),
                    });
                } else if !want_not_null && db_col.not_null && !is_pk {
                    changes.push(ColumnDiff::DropNotNull {
                        column_name: field.name.clone(),
                    });
                }

                let desired_default = field
                    .default_value
                    .as_ref()
                    .and_then(|v| json_to_sql_default(v, desired_pg_type));
                match (&db_col.default_expr, &desired_default) {
                    (None, Some(sql)) => {
                        changes.push(ColumnDiff::SetDefault {
                            column_name: field.name.clone(),
                            default_sql: sql.clone(),
                        });
                    }
                    (Some(_), None) => {
                        changes.push(ColumnDiff::DropDefault {
                            column_name: field.name.clone(),
                        });
                    }
...
fn column_diff_to_schema_change(entity: &str, diff: &ColumnDiff) -> SchemaChange {
    let (change_type, column, detail) = match diff {
        ...
        ColumnDiff::AlterType { column_name, from_type, to_type } => ("alter_type", column_name, Some(format!("{from_type} → {to_type}"))),
        ...
        ColumnDiff::ReplaceCheckConstraint { column_name, new_values, .. } => ("update_enum", column_name, Some(new_values.join(", "))),
...
mod tests {
    ...
    fn diff_add_multiple_columns() {
        ...
        let adds: Vec<_> = diff
            .changes
            .iter()
            .filter(|c| matches!(c, ColumnDiff::Add { .. }))
            .collect();
    ...
    fn diff_protected_columns_never_dropped() {
        ...
        let manifest = vec![mfield("name", "text")];
        let diff = compute_diff("app", "tbl", &db, &manifest, &empty_pk_types());
        let drops: Vec<_> = diff
            .changes
            .iter()
            .filter(|c| matches!(c, ColumnDiff::Drop { .. }))
            .collect();
    ...
    fn diff_set_not_null() {
        ...
        assert!(diff
            .changes
            .iter()
            .any(|c| matches!(c, ColumnDiff::SetNotNull { column_name } if column_name == "email")));
    }
    ...
    fn diff_drop_not_null() {
        ...
        assert!(diff
            .changes
            .iter()
            .any(|c| matches!(c, ColumnDiff::DropNotNull { column_name } if column_name == "email")));
    }
    ...
    fn diff_add_default() {
        ...
        assert!(diff
            .changes
            .iter()
            .any(|c| matches!(c, ColumnDiff::SetDefault { column_name, default_sql }
    ...
    fn diff_add_enum_check() {
        ...
        let diff = compute_diff("app", "tbl", &db, &[f], &empty_pk_types());
        let check = find_diff(&diff.changes, |c| {
            matches!(c, ColumnDiff::ReplaceCheckConstraint { .. })
        });
        match check {
            ColumnDiff::ReplaceCheckConstraint {
                old_constraint_names,
    ...
    fn diff_drop_enum_check() {
        ...
        let diff = compute_diff("app", "tbl", &db, &manifest, &empty_pk_types());
        let check = find_diff(&diff.changes, |c| {
            matches!(c, ColumnDiff::DropCheckConstraint { .. })
        });
        match check {
            ColumnDiff::DropCheckConstraint {
                constraint_names, ..
    ...
    fn ddl_add_column_format() {
        let diff = TableDiff {
            schema_name: "app".into(),
            table_name: "tbl".into(),
            changes: vec![ColumnDiff::Add {
                field: mfield("email", "text"),
                pg_type: "TEXT".into(),
            }],
        };
        let stmts = generate_ddl(&diff);
    ...
    fn ddl_statement_ordering() {
        let diff = TableDiff {
            ...
            changes: vec![
                ...
                ColumnDiff::SetDefault {
                    column_name: "role".into(),
                    default_sql: "'user'".into(),
                },
                ColumnDiff::Drop {
                    column_name: "legacy".into(),
                },
    ...
    fn column_diff_to_schema_change_all_variants() {
        let cases: Vec<(ColumnDiff, &str, &str, Option<&str>)> = vec![
            (
                ...
                "add_column", "email", Some("TEXT"),
            ),
            (
                ColumnDiff::Drop { column_name: "legacy".into() },
                "drop_column", "legacy", None,
            ),
            (
                ColumnDiff::AlterType { column_name: "score".into(), from_type: "text".into(), to_type: "DOUBLE PRECISION".into() },
                "alter_type", "score", Some("text → DOUBLE PRECISION"),
            ),
            (
                ColumnDiff::SetNotNull { column_name: "email".into() },
                "set_not_null", "email", None,
            ),
            (
                ColumnDiff::DropNotNull { column_name: "email".into() },
                "drop_not_null", "email", None,
            ),
            (
                ColumnDiff::SetDefault { column_name: "status".into(), default_sql: "'active'".into() },
                "set_default", "status", Some("'active'"),
            ),
            (
                ColumnDiff::DropDefault { column_name: "status".into() },
                "drop_default", "status", None,
            ),
            (
                ColumnDiff::ReplaceCheckConstraint { column_name: "color".into(), old_constraint_names: vec![], new_values: vec!["red".into(), "blue".into()] },
                "update_enum", "color", Some("red, blue"),
            ),
            (
                ColumnDiff::DropCheckConstraint { column_name: "color".into(), constraint_names: vec!["chk".into()] },
                "drop_enum", "color", None,
            ),
        ];
