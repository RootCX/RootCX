...
impl RuntimeExtension for AuditExtension {
    ...
    async fn bootstrap(&self, pool: &PgPool) -> Result<(), RuntimeError> {
        ...
        exec(pool, "CREATE INDEX IF NOT EXISTS idx_audit_table ON rootcx_system.audit_log (table_schema, table_name)").await?;
        ...
        exec(pool, r#"
            ...
            END;
            $$ LANGUAGE plpgsql SECURITY DEFINER"#).await?;
...
async fn list_audit_events(
    ...
    for val in &bind_values { q = q.bind(val); }
