...
impl PolicyCache {
    ...
    pub fn populate(&self, app_id: &str, contract: &PermissionsContract) {
        self.inner.write().unwrap().insert(
            ...
            CachedApp {
                ...
                policies: contract.policies.iter().map(|p| CachedPolicy {
                    role: p.role.clone(),
    ...
    pub async fn get_or_fetch(&self, pool: &PgPool, app_id: &str) -> Result<Option<CachedApp>, ApiError> {
        ...
        let role_rows: Vec<(String, Vec<String>)> = sqlx::query_as(
            "SELECT name, inherits FROM rootcx_system.rbac_roles WHERE app_id = $1",
        ...
        let default_role: Option<String> = sqlx::query_scalar(
            "SELECT manifest->'permissions'->>'defaultRole' FROM rootcx_system.apps WHERE id = $1",
        ).bind(app_id).fetch_optional(pool).await?.flatten();

        let cached = CachedApp {
            roles: role_rows.into_iter().collect(),
            policies: policy_rows.into_iter().map(|(role, entity, actions, ownership)| {
                CachedPolicy { role, entity, actions, ownership }
            }).collect(),
            default_role,
        };
...
pub fn expand_roles(assigned: &[String], role_map: &HashMap<String, Vec<String>>) -> HashSet<String> {
    ...
    while let Some(role) = stack.pop() {
        if expanded.insert(role.to_string()) {
            if let Some(parents) = role_map.get(role) {
...
pub fn detect_cycle(roles: &HashMap<String, Vec<String>>) -> Option<String> {
    ...
    for role in roles.keys() {
        if !visited.contains(role.as_str()) {
            if let Some(cycle) = dfs_cycle(role, roles, &mut visited, &mut in_stack) {
                return Some(cycle);
            }
        }
    }
    ...
    if let Some(parents) = roles.get(node) {
        for parent in parents {
            ...
            if !visited.contains(parent.as_str()) {
                if let Some(cycle) = dfs_cycle(parent, roles, visited, in_stack) {
                    return Some(cycle);
                }
            }
        }
...
pub fn evaluate(
    ...
    for p in policies {
        if !expanded_roles.contains(&p.role) { continue; }
        if p.entity != "*" && p.entity != entity { continue; }
        let action_match = p.actions.iter().any(|a| a == "*" || a == action);
        if action_match {
            allowed = true;
            if !p.ownership { has_unrestricted = true; }
        }
    }
...
pub async fn resolve_user_roles(
    ...
    let assigned: Vec<(String,)> = sqlx::query_as(
        "SELECT role FROM rootcx_system.rbac_assignments WHERE user_id = $1 AND app_id = $2",
    ).bind(user_id).bind(app_id).fetch_all(pool).await?;

    let mut roles: Vec<String> = assigned.into_iter().map(|(r,)| r).collect();
    if roles.is_empty() {
        if let Some(ref default) = cached.default_role {
...
pub async fn require_admin(
    ...
    let cached = cache.get_or_fetch(pool, app_id).await?
        .ok_or_else(|| ApiError::Forbidden("no RBAC configured".into()))?;
    let expanded = resolve_user_roles(pool, &cached, user_id, app_id).await?;
    let is_admin = cached.policies.iter().any(|p| {
        p.entity == "*" && expanded.contains(&p.role) && p.actions.iter().any(|a| a == "*")
    });
    if !is_admin {
...
mod tests {
    ...
    fn policy(role: &str, entity: &str, actions: &[&str], ownership: bool) -> CachedPolicy {
        CachedPolicy {
            role: role.into(), entity: entity.into(),
            actions: actions.iter().map(|s| s.to_string()).collect(), ownership,
        }
    }
    ...
    fn evaluate_union_semantics() {
        let p = vec![
            policy("viewer", "*", &["read"], false),
            policy("rep", "deals", &["create", "update"], true),
        ];
    ...
    fn evaluate_unrestricted_overrides_ownership() {
        ...
        let p = vec![
            policy("viewer", "deals", &["read"], false),
            policy("rep", "deals", &["read"], true),
        ];
        let r: HashSet<String> = ["viewer".into(), "rep".into()].into();
        assert_eq!(evaluate(&r, "deals", "read", &p), (true, false));
    }
    ...
    fn evaluate_all_ownership_policies() {
        ...
        let p = vec![
            policy("rep", "deals", &["read", "update"], true),
            policy("support", "deals", &["read"], true),
        ];
    ...
    fn default_role_feeds_into_evaluate() {
        ...
        let policies = vec![
            policy("viewer", "*", &["read"], false),
            policy("member", "deals", &["create"], true),
        ];
