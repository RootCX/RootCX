...
use super::policy::{resolve_user_roles, require_admin, PolicyCache};
...
pub(crate) struct RoleResponse { name: String, description: Option<String>, inherits: Vec<String> }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct AssignmentResponse { user_id: Uuid, role: String, assigned_at: chrono::DateTime<chrono::Utc> }

#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct RoleAssignment { user_id: Uuid, role: String }
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct EntityPermission { actions: Vec<String>, ownership: bool }
...
pub(crate) async fn list_roles(
    ...
    Ok(Json(rows.into_iter().map(|(name, description, inherits)| RoleResponse { name, description, inherits }).collect()))
}
...
pub(crate) async fn assign_role(
    ...
    let exists: Option<(String,)> = sqlx::query_as(
        "SELECT name FROM rootcx_system.rbac_roles WHERE app_id = $1 AND name = $2",
    ).bind(&app_id).bind(&body.role).fetch_optional(&pool).await?;
    if exists.is_none() {
...
pub(crate) async fn revoke_role(
    ...
    let r = sqlx::query(
        "DELETE FROM rootcx_system.rbac_assignments WHERE user_id = $1 AND app_id = $2 AND role = $3",
    ).bind(body.user_id).bind(&app_id).bind(&body.role).execute(&pool).await?;
...
async fn compute_permissions(
    ...
    for p in &cached.policies {
        if !expanded.contains(&p.role) { continue; }
        let entry = perms.entry(p.entity.clone()).or_insert_with(|| EntityPermission {
            actions: Vec::new(), ownership: true,
        });
        for action in &p.actions {
            if !entry.actions.contains(action) { entry.actions.push(action.clone()); }
        }
