...
impl RuntimeExtension for RbacExtension {
    ...
    async fn on_table_created(
        &self, pool: &PgPool, manifest: &AppManifest, schema: &str, table: &str,
    ) -> Result<(), RuntimeError> {
        let needs_owner = manifest.permissions.as_ref().is_some_and(|p| {
            p.policies.iter().any(|r| r.ownership && (r.entity == table || r.entity == "*"))
        });
        if !needs_owner { return Ok(()); }

        let fq = format!("{}.{}", quote_ident(schema), quote_ident(table));
        exec(pool, &format!(
            "ALTER TABLE {fq} ADD COLUMN IF NOT EXISTS owner_id UUID REFERENCES rootcx_system.users(id)"
        )).await?;
        exec(pool, &format!(
    ...
    async fn on_app_installed(
        ...
        if let Some(ref d) = contract.default_role {
            if !contract.roles.contains_key(d) {
        ...
        for (name, def) in &contract.roles {
            ...
            sqlx::query("INSERT INTO rootcx_system.rbac_roles (app_id, name, description, inherits) VALUES ($1, $2, $3, $4)")
                .bind(app_id).bind(name).bind(def.description.as_deref()).bind(&inherits)
