...
impl FromRequestParts<SharedRuntime> for AccessGrant {
    ...
    async fn from_request_parts(parts: &mut Parts, state: &SharedRuntime) -> Result<Self, Self::Rejection> {
        let (app_id, entity) =
            parse_crud_path(parts.uri.path()).ok_or_else(|| ApiError::BadRequest("invalid CRUD path".into()))?;
        ...
        let cached = match cache.get_or_fetch(&pool, &app_id).await? {
            Some(c) => c,
            None => {
                return Ok(AccessGrant { user_id: None, ownership_required: false });
            }
        };
...
mod tests {
    ...
    fn parse_crud_path_valid() {
        assert_eq!(parse_crud_path("/api/v1/apps/crm/collections/deals"), Some(("crm", "deals")),);
    }
