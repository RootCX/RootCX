...
pub async fn serve(runtime: SharedRuntime, port: u16) -> Result<(), std::io::Error> {
    let mut router = Router::new()
        ...
        .route("/api/v1/apps/{app_id}", get(routes::get_app).delete(routes::uninstall_app))
        .route("/api/v1/apps/{app_id}/icon", get(routes::get_icon))
        ...
        .route("/api/v1/crons", get(routes::list_all_crons))
    ...
    router = router.merge(crate::governance::publications::routes());
    router = router.merge(crate::governance::publication_reads::routes());
    router = router.merge(crate::governance::approved_actions::routes());
