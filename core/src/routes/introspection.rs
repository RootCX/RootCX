...
pub async fn list_schemas(
    ...
    let rows = sqlx::query_as::<_, (String, i64)>(
        "SELECT s.schema_name, COUNT(t.table_name)::bigint
         ...
         ORDER BY CASE WHEN s.schema_name = 'rootcx_system' THEN 0
                       WHEN s.schema_name = 'public' THEN 1 ELSE 2 END,
                  s.schema_name",
    )
    .fetch_all(&pool)
    .await?;
...
pub async fn list_tables(
    ...
    let columns: Vec<(String, String, String, String, Option<String>, i32)> = sqlx::query_as(
        "SELECT table_name, column_name, data_type, is_nullable,
                column_default, ordinal_position::int
...
fn pg_val(row: &PgRow, idx: usize, ti: &PgTypeInfo) -> JsonValue {
    ...
    match ti.name() {
        ...
        "TEXT[]" | "_TEXT" => try_json::<Vec<String>>(row, idx, |v| JsonValue::Array(v.into_iter().map(JsonValue::String).collect())),
        _ => row.try_get::<String, _>(idx).map(JsonValue::String).unwrap_or_else(|_| JsonValue::String(format!("<{}>", ti.name()))),
    }
}
