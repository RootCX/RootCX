...
use super::perms::detect_cycle;
use super::policy::{require_admin, resolve_permissions};
use super::perms::has_permission;
...
pub(crate) async fn list_available_permissions(
    ...
    Ok(Json(rows.into_iter()
        .filter(|(key, _)| has_permission(&caller_perms, key))
